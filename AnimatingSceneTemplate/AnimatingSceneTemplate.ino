...
const int STARTING_PERCENTAGE = 6; // Starting percentage
...
const unsigned long STEP_DELAY_MS = 750; // Delay for each step in the animation in milliseconds
...
void loop() {
  if (touch.available() && !isTouched) {
    ...
    if (x >= 100 && x <= 140 && y >= 50 && y <= 190) { // Center touch detection
